...
for i in range(NUMBER_OF_BODIES):
    body = shyphe.Body(velocity=(random.randrange(10), random.randrange(10)),
                       position=(bodies_x_spacing * (i % bodies_x), bodies_y_spacing * (i // bodies_x)))
    if random.randrange(2):
        body.add_shape(shyphe.Circle(mass=1, radius=SHAPE_SIZE // 2))
    else:
        body.add_shape(shyphe.Polygon(points=[(-SHAPE_SIZE // 2, -SHAPE_SIZE // 2),
...
while window.active():
    ...
    while world.has_next_collision():
        ...
        cola, colb = world.calculate_collision(ctr, shyphe.CollisionParameters(1))
        cola.apply_impulse()
        colb.apply_impulse()
